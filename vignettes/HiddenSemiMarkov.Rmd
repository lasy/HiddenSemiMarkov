...
vignette: >
  %\VignetteIndexEntry{Introduction to `HiddenSemiMarkov`}
...
### Loading the `HiddenSemiMarkov` package and additional libraries.
...
```{r available-marg-em, echo = FALSE}
...
mq2_fitted_to_X1 = fit_hsmm(model = mq2, X = X1, N0_emission = 0)
mq2_fitted_to_X1$model$obs_probs %>% filter(is.na(var1),  is.na(var3))


mq2_fitted_to_X3 = fit_hsmm(model = mq2, X = X3, N0_emission = 0)
...
To prevent such event, one can use the option `N0_emission` from the `fit_hsmm()` function.
...
For example, if the model is fitted on a sequence of length 500 and `N0_emission` is set to 500, then the prior and observed probabilities have the same weight when emission probabilities are re-estimated. If `N0_emission` were 1000, prior probabilities would weight twice as much.
...
m1_fitted_no_prior = fit_hsmm(model = m1, X = small_X, N0_emission = 0)
m1_fitted_small_prior = fit_hsmm(model = m1, X = small_X, N0_emission = 0.25*nrow(small_X))
m1_fitted_equal_prior = fit_hsmm(model = m1, X = small_X, N0_emission = nrow(small_X))
...
The option `N0_sojourn` works the same way but for the sojourn distributions. This option is especially useful when sojourns are specified as "non-parametric" and the model is fitted on a short sequences where each state is visited only a few times.
